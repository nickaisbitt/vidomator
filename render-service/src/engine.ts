...
try {
  ...
  const logger = winston.createLogger({
    ...
    transports: [
      ...
      ...(process.env.NODE_ENV === 'production' ? [new winston.transports.File({ filename: path.join(logsDir, 'render-service.log') })] : [new winston.transports.File({ filename: path.join(logsDir, 'render-service.log') })])
    ]
  });
  ...
  app.post('/render', async (req, res) => {
    ...
    try {
      ...
      if (!output || !segments || !Array.isArray(segments)) {
        return res.status(400).json({ error: 'Missing required fields: output, segments' });
      }
      ...
      jobQueue.push({ jobId, output, segments, music, musicVolume, thumbnail });
      setImmediate(processNextJob);
      ...
      res.status(500).json({ error: 'Failed to start render', details: error instanceof Error ? error.message : 'Unknown error' });
    }
  });
  ...
  app.post('/check-coverage', (req, res) => {
    ...
    res.json({ covered, link, title, publishedCount: published.length });
  });
  ...
  async function processVideo(
    ...
    try {
      ...
      // Render video
      await renderer.render({ output: outputPath, segments, music, musicVolume, onProgress: updateProgress });
  ...
  app.post('/fetch-visual', async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to fetch visual', details: error instanceof Error ? error.message : 'Unknown error' });
    }
  });
  ...
  app.post('/generate-seedance', async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to generate Seedance clip', details: error instanceof Error ? error.message : 'Unknown error' });
    }
  });
  ...
  app.post('/generate-free-tts', async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to generate Free TTS', details: error instanceof Error ? error.message : 'Unknown error' });
    }
  });
  ...
  app.post('/publish-to-youtube', async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Failed to upload to YouTube', details: error instanceof Error ? error.message : 'Unknown error' });
    }
  });
      ...
      try {
        ...
        try {
          ...
          const response = await axios.post(
            'https://openrouter.ai/api/v1/chat/completions',
            {
              ...
              messages: [
                { role: 'system', content: 'You are a professional broadcast news writer. Write scripts that are objective, engaging, and suitable for high-end news productions.' },
                { role: 'user', content: `Convert this news story into a 3-5 paragraph video script. Return ONLY a JSON object with this structure: { "youtubeTitle": "string", "description": "string", "tags": ["string"], "segments": [ { "text": "narration text", "visualQuery": "search term for visuals", "lowerThird": "on-screen text" } ] } Story: ${title} Content: ${content}` }
              ]
            },
            {
              headers: {
          ...
          console.error(`[ERRO] OpenRouter Failure: ${detail}`, { status: error.response?.status, jobId });
        ...
        for (let i = 0; i < scriptData.segments.length; i++) {
          ...
          try {
            ...
            logger.error(`TTS segment ${i} failed`, { jobId, error: ttsErr.message, stack: ttsErr.stack });
            seg._audioPath = null;
          }
        }
        ...
        for (let i = 0; i < visualsCount; i++) {
          ...
          try {
            ...
            if (result.success) {
              ...
              // 2. Fallback to generic stock footage
